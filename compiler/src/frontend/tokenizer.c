...
Token *read_char(char **src)
{
    ...
    if (**src == '\\')
    {
        ...
        switch (**src)
        {
            ...
            case '0':  ch = '\0'; break;
            default:{
                Error *e = invoke(UNSUPPORTED_ESC_SEQUENCE);
                appendMessageToError(e, fetchErrorMsgBasedOnErrorID(e->error_id));
                appendMessageToError(e, "Got : \\%c", **src);
                addError(e);
            }
...
Token *read_str(char **src)
{
    ...
    size_t str_size = 256;
    // Can't arena alloc bc the buffer may need to reallocate
    ...
    while (**src != '\"')
    {
        ...
        if (index + 1 >= str_size)
        {
            ...
            buffer = realloc(buffer, 2 * str_size);
        }
    }
...
Token **tokenize(char *src, char *filePath)
{
    ...
    while (*src)
    {
        ...
        // === SPECIAL HANDLING FOR < (and << shift) ===
            ...
            // Raise a warning, its not immedietaly fatal
            // printf("Raising an unexpected token error, char = %c", *src);
            Error *e = invoke(UNEXPECTED_TOKEN);
            // printf("Finished that\n");
            appendMessageToError(e, fetchErrorMsgBasedOnErrorID(e->error_id), *src);
            addError(e);
            // Continue
            src++;
        }
