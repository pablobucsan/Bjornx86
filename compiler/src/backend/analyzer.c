...
UnaryOPCode strToUnaryOPCode(char *str)
{
    ...
    if (strcmp(str, "~") == 0) { return UNARY_BIT_NOT; }
...
void validateBinaryOP(Type *left, Type *right, BinOPCode bin_op)
{
    ...
    /** ptr [bin_op] ptr allowed for comparison */
...
void __initGlobalStringTable(char *filename)
{
    gb_stringTable = makeStringTable();
    gb_stringTable->owner_filename = filename;
    gb_stringTable->filename_len = strlen(filename);
}
...
void addStrToTable(StringTable *str_table, char *str)
{
    ...
    char *tag_name = malloc(str_table->filename_len + 32);
    snprintf(tag_name, 32, "Lstr_%s_%i", str_table->owner_filename ,str_table->count);
...
Value *analyze(ASTNode *program, SymbolTable *current_st, FunctionTable *current_ft)
{
    ...
    switch (program->node_type)
    {
        ...
        case NODE_FUNC_DEF:
        {
            ...
            // printf("Func def check, errors so far: %i\n", errorManager->errors_count);
        ...
        case NODE_FUNC_CALL:
        {
            ...
            */
            // printf("In node func call, trying to look up for func = %s\n", program->funccall_node.identifier);
        ...
        case NODE_ARRAY_INIT:
        {
            ...
            // program->array_init_node.type, program->array_init_node.element_count);
                ...
                program->array_init_node.type->representation = updateTypeRepresentation(program->array_init_node.type);
                // printTypeTable();
            }
            else{
        ...
        case NODE_ASSIGNMENT:
        {
            ...
            if (isTypeOfKind(program->assignment_node.type, TYPE_ARRAY) && program->assignment_node.expression->node_type == NODE_ARRAY_INIT){
                program->assignment_node.expression->array_init_node.arr_symbol = s;
            }
