...
void genStart(FILE *file, ASTNode *program)
{
    ...
    fprintf(file, "_start:\n");
    fprintf(file, "    mov rdi, qword [rsp]\n");
    fprintf(file, "    lea rsi, [rsp + 8]\n");
...
void compile(CompilerOptions *CO)
{
    ...
    // ANALYZE AND POPULATE SYMBOL, FUNCTION, OBJECT, ENUM TABLES AND STRING TABLE
    ...
    // SECOND PASS FOR EXTERN AND _START
