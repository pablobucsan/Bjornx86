...
{
    const char *system_lib_dir = getenv("BJORN_STD_PATH");
    if (system_lib_dir == NULL) {
        const char *home = getenv("HOME");
        if (home) {
            char buf[PATH_MAX];
            snprintf(buf, sizeof(buf), "%s/bjorn-std", home);
            system_lib_dir = strdup(buf);  
        } else {
            system_lib_dir = "/home/pablo/bjorn-std";  // fallback
        }
    }
    ...
    //printf("Trying to use requested path: %s\n", requested_path);
    ...
    // Candidate 3: System standard library
    ...
    if (file_exists(candidate1)) {
        ...
        appendMessageToError(e, "Tried:\n1. '%s'\n2. '%s'\n3. '%s'", candidate1, candidate2,candidate3);
        addError(e);
    }
