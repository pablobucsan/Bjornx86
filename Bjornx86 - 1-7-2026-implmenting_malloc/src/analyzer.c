...
{
    ContextType current_ctx = getCurrentContext();
    ...
    switch (current_ctx)
    {
        ...
        case CTX_ELSE:
        {
            if (ctx_type == CTX_ENUM || ctx_type == CTX_OBJECT || ctx_type == CTX_FUNCTION || ctx_type == CTX_CLASS)
            {
                return 0;
            }
        }
...
void pushContext(ContextType ctx_type)
{
    ...
    if (!isAllowedContext(ctx_type)){
        ...
        appendMessageToError(e, fetchErrorMsgBasedOnErrorID(e->error_id), ctxToString(ctx_type), ctxToString(getCurrentContext()));
        addError(e);
    }
...
void popContext()
{
    ...
    ContextType ctx_type = getCurrentContext();
...
{   
    ...
    snprintf(counter_name, 32, "__foreach_counter_%i", getCurrentContextBlockId(CTX_FOREACH));
    Type *counter_type = createNumberTypeFromInfo(4,1, NUMBER_INTEGER);
    ...
    ASTNode *ast_subscript = makeSubscript(ast_foreach->foreach_node.iterable_expr, counter_id_subscript, ast_foreach->foreach_node.iterable_expr_type,
                                            calculateSizeOfType(ast_foreach->foreach_node.iterator_type),4);

    //printf("HERE\n");
    ASTNode *iterator_reassignment = makeReassignment(ast_iterator_id, TOKEN_ASSIGN, ast_subscript,ast_foreach->foreach_node.iterator_type );
    ...
    ASTNode *ast_incr_counter = makePostFix(counter_id_incr, counter_type, 4, "++");
...
{  
    Type *ptype = type->pointerType.pointed_type;

    while (isTypeOfKind(ptype, TYPE_PTR) && ptype->kind != TYPE_VOID){
        ptype = ptype->pointerType.pointed_type;
    }
    return ptype;
}
...
int typesMatchForFunctionCall(Type *type, Type *providedType)
{
    ...
    if ( (isTypeOfKind(type, TYPE_PTR) && isTypeOfKind(type->pointerType.pointed_type, TYPE_NUMBER) && type->pointerType.pointed_type->numberType.kind == NUMBER_CHAR && 
        isTypeOfKind(providedType, TYPE_STR)) || 
         (isTypeOfKind(providedType, TYPE_PTR) && isTypeOfKind(providedType->pointerType.pointed_type, TYPE_NUMBER) && providedType->pointerType.pointed_type->numberType.kind == NUMBER_CHAR && 
        isTypeOfKind(type, TYPE_STR))){
        return 1;
    }
...
int isTypeAssignableToDeclared(Type *declaredType, Type *assigningType)
{
    ...
    /** Str and ptr<char> are the same */
    if ( (isTypeOfKind(declaredType, TYPE_PTR) && isTypeOfKind(declaredType->pointerType.pointed_type, TYPE_NUMBER) && declaredType->pointerType.pointed_type->numberType.kind == NUMBER_CHAR && 
        isTypeOfKind(assigningType, TYPE_STR)) || 
         (isTypeOfKind(assigningType, TYPE_PTR) && isTypeOfKind(assigningType->pointerType.pointed_type, TYPE_NUMBER) && assigningType->pointerType.pointed_type->numberType.kind == NUMBER_CHAR && 
        isTypeOfKind(declaredType, TYPE_STR))){
        return 1;
    }
     ...
     */
    if (assigningType->kind != TYPE_VOID && declaredType->kind != assigningType->kind){
        return 0;
    }
    ...
    switch(assigningType->kind)
    {
        ...
        case TYPE_UDT:
        {
             ...
             */
            if (assigningType->udt.udt_kind != assigningType->udt.udt_kind){
                return 0;
            }
            /**
        ...
        case TYPE_STR:{
            /**
             * Just a change of where the str points to, is fine
             */
            return 1;
        }
...
void validateBinaryOP(Type *left, Type *right, BinOPCode bin_op)
{
     ...
     */
    if ( (left->kind == TYPE_PTR && left->pointerType.pointed_type->kind != TYPE_VOID && right->kind == TYPE_NUMBER) || 
        ( left->kind == TYPE_NUMBER && right->kind == TYPE_PTR && right->pointerType.pointed_type->kind != TYPE_VOID)){
        switch (bin_op)
        {
            case ADD:
            case SUB: return;
...
int doesTypeExist(Type *type)
{
    switch(type->kind)
    {
        ...
        case TYPE_ARRAY:
        {
            Type *element_type = type->arrayType.element_type;
            return doesTypeExist(element_type);
        }
        case TYPE_PTR:
        {
            Type *pointed_type = type->pointerType.pointed_type;
            return doesTypeExist(pointed_type);
        }
        case TYPE_UDT:
        {
...
void checkConflictingUDTs(char *udt_name)
{
        ...
        switch(gb_typeTable->types[i]->kind)
        {
            ...
            case TYPE_PTR:{
                ...
                if (t->kind == TYPE_UDT && strcmp(t->udt.name, udt_name) == 0){
                    t->udt.udt_kind = udt_kind;
                }
                break;
            }
            case TYPE_UDT:{
                if (strcmp(gb_typeTable->types[i]->udt.name, udt_name) == 0){
                    gb_typeTable->types[i]->udt.udt_kind = udt_kind;
                }
                break;
            }
            default:{
...
Symbol *makeLocalVarSymbol(char *identifier, Type *type)
{
    ...
    s->s_local.symbolLocalVar.context_block_id = getCurrentContextBlockId(getCurrentContext());
    s->s_local.symbolLocalVar.definition_context = getCurrentContext();
    ...
    while (st != NULL){
        ...
        while (checking_scope >= 0){
            for (int i = 0; i < st->count; i++){
                 ...
                 */
                if (st->symbols[i]->smi->scope != checking_scope) { 
                    continue; 
                }
...
int doSymbolsClash(Symbol *s1, Symbol *s2)
{
     ...
     * If they are globals, not allowed
     ...
     */
    if (s1->s_local.symbolLocalVar.definition_context != s2->s_local.symbolLocalVar.definition_context){
        return 0;
    }
     ...
     */
    if (s1->s_local.symbolLocalVar.context_block_id != s2->s_local.symbolLocalVar.context_block_id){
        return 0;
    }
...
Function *makeSingleFunction(char *name, int param_count, Parameter **parameters, Type *rt_type, FunctionTable *parentFT, SymbolTable *parentST)
{
    Function *f = __allocateFunction();
    f->fmi = makeFunctionMetaInfo(name, param_count, parameters, rt_type, parentFT, parentST);
...
Function *selectBestFunctionCandidateOf(Function *f1, Function *f2, Type **param_types, int param_count)
{
    ...
    for (int i = 0; i < param_count; i++)
    {
        ...
        if (!isTypeOfKind(t1, TYPE_UDT)) { continue; }
...
Function *lookUpFunction(FunctionTable *ft, int scope, char *identifier, Type **param_types, int param_count)
{
    ...
    //printf("Looking for f = %s\n", identifier);
    FunctionTable *current_ft = ft;
    while (current_ft != NULL){
        //printf("Looking in the following ft:\n");
        ...
        while (checking_scope >= 0)
        {
            for (int i = 0; i < current_ft->count; i++){
                 ...
                 */
                //printf("Current f = %s, has scope = %i \n", current_ft->functions[i]->fmi->name, current_ft->functions[i]->fmi->scope);
    ...
    //printf("We have %i candidates for func = %s\n", fc->count, identifier);
...
Object *makeObject(char *objectName, Object *parentObject)
{
...
{
    checkConflictingUDTs(o->identifier);
...
Class *makeClass(char *name, Class *parentClass)
{
    ...
    if (parentClass != NULL){
        ...
        c->local_functions = makeFunctionTable(NULL);
        c->local_symbols = makeSymbolTable(NULL, KIND_CLASS_ST);
    }
    //printTypeTable();
...
void addClassToTable(ClassTable *ct, Class *c)
{
    ...
    checkConflictingUDTs(c->identifier);
...
{
    checkConflictingUDTs(e->identifier);
...
Value *analyze(ASTNode *program, SymbolTable *current_st, FunctionTable *current_ft)
{
    ...
    //printf("L = %i. Analyzing node with type: %s \n", tracker.current_line, astTypeToStr(program));
    ...
    switch (program->node_type)
    {
        ...
        case NODE_EXTERN_FUNC_DEF:
        {
            ...
            Function *f = makeSingleFunction(program->extern_func_def_node.func_name, program->extern_func_def_node.params_count, 
                                             program->extern_func_def_node.params, program->extern_func_def_node.return_type, current_ft, current_st);
        ...
        case NODE_FUNC_DEF:
        {
            ...
            if (c == NULL){
                f = makeSingleFunction(program->funcdef_node.func_name, program->funcdef_node.params_count,
                                       program->funcdef_node.params, program->funcdef_node.return_type, current_ft, current_st);
            }
            else{
                Type *c_type = makeClassType(c->identifier);
        ...
        case NODE_IF:
        {
            ...
            Value *if_value = analyze(program->if_node.body, current_st, current_ft);
            ...
            for (int i = 0; i < program->if_node.elseif_count; i++){
                ...
                Tag *elseif_tag = makeTag("elseif_stmt_%i", getCurrentContextBlockId(CTX_ELSEIF));
        ...
        case NODE_ELSEIF:
        {
            ...
            Value *elseif_value = analyze(program->elseif_node.body, current_st, current_ft);
        ...
        case NODE_FOREACH:
        {
            ...
            if (!areTypesEqual(element_type, iterator_declaration_type)){
        ...
        case NODE_SIZEOF:
        {
            int exists = doesTypeExist(program->sizeof_node.type);
            ...
            else if (isTypeOfKind(program->sizeof_node.type, TYPE_ARRAY) && program->sizeof_node.type->arrayType.size_provided_kind == ARR_SIZE_INTERPRETED){
            ...
            int size = calculateSizeOfType(program->sizeof_node.type);
            Type *sizeof_type = createNumberTypeFromInfo(8,1, NUMBER_INTEGER);   // uint64
            Value *result = makeSpecificValue(sizeof_type);
            //printf("Type resulting from sizeof: %s\n", typeToString(sizeof_type));
            return result;
        }
        ...
        case NODE_METHOD_DISPATCH:
        {
            ...
            program->method_dispatch.base_type = makeClassType(c->identifier);
        ...
        case NODE_OBJECT:
        {
             ...
             */
            Object *o = makeObject(program->object_node.identifier, parent);
            if (parent != NULL){
        ...
        case NODE_CLASS:
        {
            ...
            Class *c = makeClass(program->class_node.identifier, parent);
        ...
        { 
            ...
            Enum *e = makeEnum(program->enum_node.identifier);


            addEnumToTable(gb_enumTable, e);

            //What to analyze? It's just identifiers here, and manually assigned to uint8 values by the compiler
            for (int i = 0; i < program->enum_node.declaration_count; i++)
            {
                analyze(program->enum_node.declarations[i], current_st, current_ft);
            }
            popContext();
        ...
        case NODE_ASSIGNMENT:
        {
            ...
            if (isTypeOfKind(program->assignment_node.type, TYPE_ARRAY) && program->assignment_node.expression->node_type == NODE_ARRAY_INIT){
                if (program->assignment_node.type->arrayType.n_of_elements != v->type->arrayType.n_of_elements){
                    Error *e = invoke(ARRAY_SIZE_MISMATCH);
                    appendMessageToError(e, fetchErrorMsgBasedOnErrorID(e->error_id));
                ...
                int match = isTypeAssignableToDeclared(s->smi->type, v->type);
                if (!match){
                    Error *e = invoke(RTYPE_CANT_BE_ASSIGNED_TO_LTYPE);
                    appendMessageToError(e, fetchErrorMsgBasedOnErrorID(e->error_id), typeToString(s->smi->type), typeToString(v->type));
                    addError(e);
                }
            }
