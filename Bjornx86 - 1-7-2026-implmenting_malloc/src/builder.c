...
void collectStrings(FILE *asm_file, ASTNode *program)
{
    ...
    switch(program->node_type)
    {
        ...
        case NODE_FOREACH:
        {
            collectStrings(asm_file, program->foreach_node.foreach_declaration);
            collectStrings(asm_file, program->foreach_node.iterable_expr);
            collectStrings(asm_file, program->foreach_node.lower_limit_expr);
            collectStrings(asm_file, program->foreach_node.upper_limit_expr);
...
Operand * buildStart(FILE *asm_file, ASTNode *program, SymbolTable *current_st, FunctionTable *current_ft)
{
    ...
    {   
        ...
        case NODE_FUNC_CALL:
        {
            ...
            // printf("About to call a function with name: %s, and params:\n", program->funccall_node.identifier);
            // for (int i = 0; i < program->funccall_node.params_count; i++){
            //     printf("%s\n", typeToString(program->funccall_node.params_type[i]));
            // }
            ...
            for (int i = 0; i < program->funccall_node.params_count; i++)
            {
                ...
                if (i >= 6)
                {
                    ...
                    expectRegister(GPR, calculateSizeOfType(program->funccall_node.params_type[i]));
