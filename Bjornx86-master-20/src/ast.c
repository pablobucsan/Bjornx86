...
//(type)
int is_cast_type(Token **tokens, int *token_pos)
{
    int token_pos_copy = *token_pos;
    if (strcmp(tokens[token_pos_copy]->tk_value, "(") != 0) { return 0; }
    ...
    {
        fprintf(stderr, "Type:'%s' is not castable.\n", type);
        exit(1);
    }

    
    if (type != NULL && strcmp(tokens[token_pos_copy]->tk_value, ")") == 0)
    {
        return 1;
    }
...
int is_assignment(Token **tokens, int token_pos)
{
    ...
    if (tokens[token_pos_copy]->tk_type != TOKEN_IDENTIFIER && tokens[token_pos_copy]->tk_type != TOKEN_KEYWORD && 
    tokens[token_pos_copy]->tk_type != TOKEN_TYPE) 
    { 
        //printf("It's not an assignment\n"); 
        return 0; 
    }

    char *type = tryResolveType(tokens, &token_pos_copy);
    if (tokens[token_pos_copy]->tk_type == TOKEN_IDENTIFIER && strcmp(tokens[token_pos_copy + 1]->tk_value, "=") == 0) 
    { 
...
int is_reassignment(Token **tokens, int token_pos)
{
    ...
    // If we find "),]" without finding first "(,[", its not
    // There can be "," for func calls so..
    while (tokens[i]->tk_type != TOKEN_EOF && strcmp(tokens[i]->tk_value, ";") != 0)
    {

        if (is_assignable_op(tokens, i)) 
        { //printf("It is \n"); 
            return 1; 
        }

        if (strcmp(tokens[i]->tk_value, "(") == 0) { brackets++; i++; continue; }
        if (strcmp(tokens[i]->tk_value, "[") == 0) { sqbrackets++; i++; continue; }

        if (strcmp(tokens[i]->tk_value, ")") == 0)
        {
            brackets--;
            if (brackets == -1) { return 0; } // We found more closening brackets than opening
        }

        if (strcmp(tokens[i]->tk_value, "]") == 0)
        {
            sqbrackets--;
            if (sqbrackets == -1) { return 0; } // We found more closening sqbrackets than opening
        }

        if (strcmp(tokens[i]->tk_value, "{") == 0 || strcmp(tokens[i]->tk_value, "}") == 0) { return 0; } //"{}" are not part of reassignments

        i++;

        //printf("In is_reassignment(). Current token = %s\n", tokens[i]->tk_value);
    }
...
ASTNode *cloneAST(ASTNode *original)
{
    ...
    switch (original->node_type)
    {
        ...
        {
            clone->assignment_node.identifier = strdup(original->assignment_node.identifier);
            clone->assignment_node.line_number = original->assignment_node.line_number;
            clone->assignment_node.type = strdup(original->assignment_node.type);
        ...
        case NODE_REASSIGNMENT:
        {
            ...
            clone->reassignment_node.size = original->reassignment_node.size;
            clone->reassignment_node.type = strdup(original->reassignment_node.type);
            break;
        }
        ...
        case NODE_SUBSCRIPT:
        {
            ...
            clone->subscript_node.base_identifier = strdup(original->subscript_node.base_identifier);
            clone->subscript_node.base_type = strdup(original->subscript_node.base_type);
        ...
        {
            clone->array_init_node.arr_name = strdup(original->array_init_node.arr_name);
            ...
            clone->array_init_node.size = original->array_init_node.size;
            clone->array_init_node.type = strdup(original->array_init_node.type);
            break;
        }
        ...
        case NODE_PTR_FIELD_ACCESS:
        {
            ...
            clone->ptr_field_access_node.field_name = strdup(original->ptr_field_access_node.field_name);
            clone->ptr_field_access_node.type = strdup(original->ptr_field_access_node.type);
        ...
        case NODE_FIELD_ACCESS:
        {
            ...
            clone->field_access_node.field_name = strdup(original->field_access_node.field_name);
            clone->field_access_node.type = strdup(original->field_access_node.type);
            break;
        ...
        {
            clone->declaration_node.identifier = strdup(original->declaration_node.identifier);
            clone->declaration_node.type = strdup(original->declaration_node.type);
            break;
        }

        case NODE_UNARY_OP:
        {
            clone->unary_op_node.op = strdup(original->unary_op_node.op);
        ...
        case NODE_POSTFIX_OP:
        {
            ...
            clone->postfix_op_node.op = strdup(original->postfix_op_node.op);
            clone->postfix_op_node.size_of_operand = original->postfix_op_node.size_of_operand;
            clone->postfix_op_node.type = strdup(original->postfix_op_node.type);
            break;
        }

        case NODE_STDALONE_POSTFIX_OP:
        {
            clone->stdalone_postfix_op_node.left = cloneAST(original->stdalone_postfix_op_node.left);
            clone->stdalone_postfix_op_node.op = strdup(original->stdalone_postfix_op_node.op);
            clone->stdalone_postfix_op_node.size_of_operand = original->stdalone_postfix_op_node.size_of_operand;
            clone->stdalone_postfix_op_node.type = strdup(original->stdalone_postfix_op_node.type);
            break;
        }

        case NODE_CAST:
        {
            clone->cast_node.castType = strdup(original->cast_node.castType);
        ...
        case NODE_BINARY_OP:
        {
            ...
            clone->binary_op_node.op = strdup(original->binary_op_node.op);
        ...
        case NODE_NUMBER:
        {
            clone->number_node.int_type = strdup(original->number_node.int_type);
        ...
        case NODE_STR:
        {
            clone->str_node.label = strdup(original->str_node.label);
            clone->str_node.str_value = strdup(original->str_node.str_value);
            break;
        }

        case NODE_BOOL:
        {
            clone->bool_node.bool_value = strdup(original->bool_node.bool_value);
            break;
        }

        case NODE_IDENTIFIER:
        {
            clone->identifier_node.name = strdup(original->identifier_node.name);
            break;
        }
        ...
        case NODE_FOREACH:
        {
            ...
            clone->foreach_node.counter_name = strdup(original->foreach_node.counter_name);
            ...
            clone->foreach_node.iterable_expr_type = strdup(original->foreach_node.iterable_expr_type);
            clone->foreach_node.iterator_name = strdup(original->foreach_node.iterator_name);
            clone->foreach_node.iterator_type = strdup(original->foreach_node.iterator_type);
        ...
        case NODE_FUNC_CALL:
        {
            clone->funccall_node.identifier = strdup(original->funccall_node.identifier);
            clone->funccall_node.is_callback = original->funccall_node.is_callback;
            ...
            for (int i = 0; i < clone->funccall_node.params_count; i++)
            {
                clone->funccall_node.params_type[i] = strdup(original->funccall_node.params_type[i]);
            }
            break;
        ...
        {
            clone->stdalone_func_call_node.identifier = strdup(original->stdalone_func_call_node.identifier);
            clone->stdalone_func_call_node.is_callback = original->stdalone_func_call_node.is_callback;
            clone->stdalone_func_call_node.params_count = original->stdalone_func_call_node.params_count;
            
            clone->stdalone_func_call_node.params_expr = malloc(sizeof(ASTNode *) * clone->stdalone_func_call_node.params_count);
            for (int i = 0; i < clone->stdalone_func_call_node.params_count; i++)
            {
                clone->stdalone_func_call_node.params_expr[i] = cloneAST(original->stdalone_func_call_node.params_expr[i]);
            }
            
            clone->stdalone_func_call_node.params_type = malloc(sizeof(char *) * clone->stdalone_func_call_node.params_count);
            for (int i = 0; i < clone->stdalone_func_call_node.params_count; i++)
            {
                clone->stdalone_func_call_node.params_type[i] = strdup(original->stdalone_func_call_node.params_type[i]);
            }
            break;
        }
...
{
    ASTNode *ast_assignment = makeAST();
...
{
    ASTNode *ast_reassignment = makeAST();
    ...
    ast_reassignment->reassignment_node.size = size;
...
{
    ASTNode *ast_stdalonePF = makeAST();

    ast_stdalonePF->node_type = NODE_STDALONE_POSTFIX_OP;
    ast_stdalonePF->stdalone_postfix_op_node.left = left;
    ast_stdalonePF->stdalone_postfix_op_node.type = type;
    ast_stdalonePF->stdalone_postfix_op_node.size_of_operand = size_of_operand;
    ast_stdalonePF->stdalone_postfix_op_node.op = op;
...
ASTNode *parseStatement(Token **tokens, int *token_pos)
{
            ...
            && tokens[*token_pos + 1]->tk_type == TOKEN_IDENTIFIER && strcmp(tokens[*token_pos + 2]->tk_value, "{") == 0)
    {
        ...
        while (strcmp(tokens[*token_pos]->tk_value, "}") != 0)
        {
            ...
            ast_enum_decl->declaration_node.type = ast_enum->enum_node.identifier;
...
char *tryResolveType(Token **tokens, int *token_pos)
{
    ...
    if (tokens[*token_pos]->tk_type == TOKEN_SYMBOL && strcmp(tokens[*token_pos]->tk_value, "[") == 0)
    {
        ...
        {
            // Not a declaration nor assignment if the array is not declared with number size.
            return 0;
        }
...
//Expression handles '&&'. Precedence 9.
...
//Expr handles BITWISE OR "|". Precedence 8
...
//Expr handles BITWISE AND "&". Precedence 7
...
ASTNode *parseExpr_Precedence1(Token **tokens, int *token_pos)
{
            ...
            strcmp(tokens[*token_pos]->tk_value, "--") == 0)))
    {
        ...
        if (strcmp(tokens[*token_pos]->tk_value, "[") == 0)
        {
            ...
            ast_subscript->subscript_node.base = base;
            ast_subscript->subscript_node.base_type = "";
...
ASTNode *parseUnit(Token **tokens, int *token_pos)
{
    ...
    else if (is_cast_type(tokens, token_pos))
    {
